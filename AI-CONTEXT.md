...
**部署现状**：原腾讯云托管地址 `https://classroom-poll-294902-10-1304972958.sh.run.tcloudbase.com` **因配额耗尽已停用**；现部署在**阿里云 ECS —— Alibaba Cloud Linux 3（阿里云 Linux，Docker 方式）**，见 §3 与 §12.4，一键脚本 `deploy/aliyun-ecs/deploy.sh`、完整指南 `deploy/aliyun-ecs/配置说明.md`（含该系统的 Docker 安装命令）。另有 Windows Server 方案 `deploy/windows-server/` 备选。
...
- **当前部署目标：阿里云 ECS —— 实际系统为 Alibaba Cloud Linux 3**（Docker 方式，见 `deploy/aliyun-ecs/`：`deploy.sh` 一键构建+运行、`配置说明.md` 含该系统的 Docker 安装命令、`nginx.conf` HTTPS 反代示例）。ECS 磁盘本身持久：`DATA_DIR=/data` 直接落盘，**无需额外挂载云盘/持久卷**；容器 `-p 80:80` 映射、`--restart=always`；安全组放行 80/443。Windows Server 方案（`deploy/windows-server/`）为备选，已弃用不用。
