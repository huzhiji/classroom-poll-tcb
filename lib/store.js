...
// 一次性导入「课堂习题」系列专题（来自 course_output 的做题网站题库）。
// 幂等：仅当 db.meta.importedClassroom 未置位时导入一次，之后不再重复。
function seedClassroomQuestions() {
  if (db.meta && db.meta.importedClassroom) return;
  let items;
  try {
  ...
  persist();
  console.log(`[store] 已导入课堂习题：${Object.keys(byTopic).map((t) => t + '(' + byTopic[t].length + ')').join('、')}`);
}
...
function getLawStats() {
  ...
  // 9) 每题「全时段」正确率（含历史累计）：用于题目列表内联展示，覆盖所有曾被作答的题目（不限当前推送）
  const qrateAgg = {};
  Object.keys(db.studentAnswers || {}).forEach((key) => {
    (db.studentAnswers[key] || []).forEach((ev) => {
