...
function getAIConfig() {
  db.meta = db.meta || {};
  // 默认优先使用本地大模型（Ollama 等），无需联网、不依赖第三方账号；
  // 如需云端，可在教师端改成通义/DeepSeek/七牛等 OpenAI 兼容地址。
  const def = db.meta.aiConfig || {};
  const endpoint = def.endpoint || 'http://127.0.0.1:11434/v1';
  return {
    endpoint,
    key: def.key || '',
    model: def.model || 'qwen2.5:7b',
    enabled: !!def.enabled,
  };
}
