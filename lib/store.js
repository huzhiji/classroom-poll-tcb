...
function seedClassroomQuestions() {
  ...
  const byTopic = {};
  let added = 0;
  const classroomOnlyTopics = new Set(
    items.filter((it) => it.classroomOnly).map((it) => it.topic || '未分类')
  );
  items.forEach((it) => {
    ...
    const created = addQuestion(it);
    if (!created || !created.title) return;
    added++;
    if (it.classroomOnly) return; // 仅入题库（供「课堂练习」推送），不生成专题考试
    const t = it.topic || '未分类';
    byTopic[t] = byTopic[t] || [];
    byTopic[t].push(created.id);
  });
  Object.keys(byTopic).forEach((t) => {
    if (!byTopic[t].length) return;
  ...
  // 清理：标记 classroomOnly 的专题不应出现在「考试答题」，删除其历史专题考试（幂等，重部署自动修正）
  if (classroomOnlyTopics.size) {
    const before = (db.exams || []).length;
    db.exams = (db.exams || []).filter(
      (e) => !(e.mode === 'topic' && classroomOnlyTopics.has(e.title))
    );
    if (db.exams.length !== before) {
      console.log(`[store] 已移除 ${before - db.exams.length} 个 classroomOnly 专题考试（不进入考试板块）`);
    }
  }
  db.meta = db.meta || {};
