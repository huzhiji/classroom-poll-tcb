...
function getAIConfig() {
  db.meta = db.meta || {};
  // 默认预填可选的大模型接入信息（endpoint/model），key 需教师自行填写；
  // 注意：下方默认 endpoint 的账号若被服务商禁用 chat，调用会返回 403，需更换可用账号。
  const def = db.meta.aiConfig || {};
  return {
    endpoint: def.endpoint || 'https://api.qnaigc.com/v1',
    key: def.key || '',
    model: def.model || 'deepseek/deepseek-v4-pro-0813',
    enabled: !!def.enabled,
  };
}
