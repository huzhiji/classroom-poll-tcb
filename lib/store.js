...
function getAIConfig() {
  db.meta = db.meta || {};
  return db.meta.aiConfig || { endpoint: '', key: '', model: '', enabled: false };
}
function saveAIConfig(cfg) {
  ...
  db.meta.aiConfig = {
    ...
    enabled: !!(cfg.enabled && String(cfg.endpoint || '').trim() && String(cfg.key || '').trim()),
  };
  persist();
...
function aggregateStudent(stu, periodStart, periodEnd) {
  ...
  (stu.history || []).forEach((h) => {
    ...
    (h.details || []).forEach((d) => {
      ...
      const c = classifyQ(d.qid);
      const isC = d.isCorrect === true;
      byCat[c.label] = byCat[c.label] || { total: 0, correct: 0, label: c.label };
      byCat[c.label].total++;
      if (isC) byCat[c.label].correct++;
      const qq = getAnyQuestion(d.qid);
      byQ[d.qid] = byQ[d.qid] || { total: 0, correct: 0, title: (qq && qq.title) || d.qid };
      byQ[d.qid].total++;
      if (isC) byQ[d.qid].correct++;
  ...
  return { total, correct, accuracy: total ? correct / total : 0, cats, byQ };
}
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  try {
    ...
    const resp = await fetch(`${cfg.endpoint.replace(/\/$/, '')}/chat/completions`, {
      ...
      body: JSON.stringify({ model: cfg.model || 'gpt-3.5-turbo', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], temperature: 0.6 }),
      signal: ctrl.signal,
    });
    clearTimeout(t);
...
module.exports = {
  // 题目  listQuestions, getQuestion, addQuestion, addQuestionsBatch, parseQuestionsText,
