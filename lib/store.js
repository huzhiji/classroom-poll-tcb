...
let db = {
  ...
  studentAnswers: {}, // 每名学生原始答题流水 key->[每条作答]（给每个人建库，任何数据都留底，含第几次提交/批次/耗时/得分等）
  studentAttempts: {}, // 提交序号计数器 key->{seq, byRef:{`kind::refId`: n}}，用于计算「第几次提交」
  reports: [],     // 学习周报/月报 [{id,type,periodStart,periodEnd,createdAt,classSummary,students,aiText,aiSource}]
  aiChats: {},     // AI 对话历史 { chatKey: [{role,content,ts}] }（按学生/访客持久保存）
  points: {},      // 学生积分 { studentKey: { balance, examDone:{}, correctQ:{}, log:[] } }（考试/做题赚取，深度模型消耗）
...
function loadFromDisk() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      db.studentAttempts = obj.studentAttempts || {};
      db.reports = obj.reports || [];
      db.aiChats = obj.aiChats || {};
      db.points = obj.points || {};
...
seedShenlun();
seedSpecialTopics();
migrateClassroomTopics(); // 历史「课堂习题·」考试专题 → 课堂练习集(mode:classroom)，须在 seedClassroomQuestions 之前
...
function seedSpecialTopics() {
  ...
  // 专题标题直接使用题库里的 topic（已含「公基常识·」前缀，前缀约定在 JSON 中维护），
  // find 按精确标题匹配保证幂等，不重复生成。
  Object.keys(byTopic).forEach((t) => {
    if (!byTopic[t].length) return;
    const exam = db.exams.find((e) => e.mode === 'topic' && e.title === t);
    if (exam) {
...
function seedClassroomQuestions() {
  ...
  // 内容指纹：topic + options + correctAnswer。用于「题干(title)被改、但选项与答案未变」时
  // 原位更新（保留原 id），避免重新导入产生重复题 / 旧错题残留在题库中（习题与解析串了的修复场景）。
  const contentMap = {};
  (db.questions || []).forEach((q) => {
    const key = String(q.topic || '') + '␟' + JSON.stringify(q.options || []) + '␟' + String(q.correctAnswer);
    if (!contentMap[key]) contentMap[key] = q;
  });
  let added = 0, updated = 0;
  items.forEach((it) => {
    const title = String(it.title || '').trim();
    if (!title) return;
    if (existingTitles.has(title)) return; // 标题去重
    const ckey = String(it.topic || '') + '␟' + JSON.stringify(it.options || []) + '␟' + String(it.correctAnswer);
    const hit = contentMap[ckey];
    if (hit) {
      // 题干被改动（如旧标题误填成解析），原位更新，避免重复导入
      hit.title = title;
      if (it.analysis != null) hit.analysis = it.analysis;
    ...
    existingTitles.add(title);
    const created = addQuestion(it);
    if (created && created.title) added++;
  });
  // 幂等确保课堂习题专题考试存在：按 topic 聚合 db.questions 中全部题目（含历史已导入），
  // 即使题目已存在也重建/补全专题，避免部署后专题考试缺失（自我修复）。
  const seedTopics = [...new Set(items.map((it) => it.topic || '未分类'))];
  const byTopic = {};
  (db.questions || []).forEach((q) => {
    if (seedTopics.includes(q.topic)) {
      byTopic[q.topic] = byTopic[q.topic] || [];
      byTopic[q.topic].push(q.id);
    }
  });
  // 幂等补全/重建「课堂练习集」：标题直接使用题库里的 topic（已含「课堂习题·」前缀）。
  // mode:'classroom' = 课堂模块练习集（学生端「课堂」tab 可见、教师端按专题勾选发布）；
  // 不再生成 mode:'topic'，以免这些课堂题库专题出现在「考试」模块。
  seedTopics.forEach((t) => {
    if (!byTopic[t] || !byTopic[t].length) return;
    const exam = db.exams.find((e) => e.mode === 'classroom' && e.title === t);
    if (exam) {
      const set = new Set(exam.questionIds || []);
      byTopic[t].forEach((id) => set.add(id));
      exam.questionIds = [...set];
  ...
  if (added > 0 || updated > 0) {
    console.log(`[store] 已追加课堂习题 ${added} 题、原位更新 ${updated} 题：${Object.keys(byTopic).map((t) => t + '(+' + byTopic[t].length + ')').join('、')}`);
  } else {
    console.log('[store] 课堂习题已是最新，无需导入');
  }
}

// 一次性迁移：历史由课堂题库生成的 mode:'topic' 专题（标题以「课堂习题·」开头）
// 改为 mode:'classroom'（课堂练习集），使其从「考试」模块移到「课堂」模块。
function migrateClassroomTopics() {
  let n = 0;
  (db.exams || []).forEach((e) => {
    if (e.mode === 'topic' && String(e.title || '').startsWith('课堂习题·')) { e.mode = 'classroom'; n++; }
  });
  if (n) { persist(); console.log(`[store] 已将 ${n} 个「课堂习题·」专题迁移为课堂练习集（课堂模块）`); }
  return n;
}

// ========== 课堂练习（课堂模块：按专题浏览题库 → 教师勾选发布 → 学生端「课堂」练习） ==========
// 题库：按 topic 分组返回「课堂习题·」系列全部题目，供教师端浏览勾选
function getClassroomBank() {
  const byTopic = {};
  (db.questions || []).forEach((q) => {
    const t = String(q.topic || '');
    if (!t.startsWith('课堂习题·')) return;
  ...
  return Object.keys(byTopic).sort().map((topic) => ({
    topic, count: byTopic[topic].length, questions: byTopic[topic],
  }));
}
// 已发布的课堂练习集
function getClassroomPush() {
  return (db.exams || []).filter((e) => e.mode === 'classroom').map((e) => {
    const items = (e.items && e.items.length)
      ? e.items
      : (e.questionIds || []).map((qid) => ({ qid, showAnswer: true, showAnalysis: true }));
    return {
      id: e.id, title: e.title, count: items.length, pushedAt: e.createdAt || 0,
      ansVisible: items.filter((i) => i.showAnswer !== false).length,
      anaVisible: items.filter((i) => i.showAnalysis !== false).length,
      items,
    };
  });
}
// 发布/更新：list=[{topic, qids:[]}]；同一专题重复发布会整体替换其题目集合
// 发布/更新：list=[{topic, qids:[] 或 configs:[{qid, showAnswer, showAnalysis}]}]
// 同一专题重复发布会整体替换其题目集合与逐题可见性配置
function addClassroomPush(list) {
  (list || []).forEach((g) => {
    if (!g || !g.topic) return;
    const topic = String(g.topic);
    const valid = new Set((db.questions || []).filter((q) => q.topic === topic).map((q) => String(q.id)));
    let items;
    if (Array.isArray(g.configs) && g.configs.length) {
      items = g.configs.filter((c) => c && valid.has(String(c.qid))).map((c) => ({
        qid: String(c.qid), showAnswer: c.showAnswer !== false, showAnalysis: c.showAnalysis !== false,
      }));
    } else {
      const showAnswer = g.showAnswer !== false, showAnalysis = g.showAnalysis !== false;
      items = (g.qids || []).map(String).filter((id) => valid.has(id)).map((qid) => ({ qid, showAnswer, showAnalysis }));
    }
    if (!items.length) return;
    const exam = db.exams.find((e) => e.mode === 'classroom' && e.title === topic);
    if (exam) {
      exam.questionIds = items.map((i) => i.qid);
      exam.items = items;
    } else {
      db.exams.push({
        id: nowId('e', 'eSeq'), title: topic, mode: 'classroom',
        questionIds: items.map((i) => i.qid), items, createdAt: Date.now(),
      });
    }
  });
  persist();
  return getClassroomPush();
}
// 取某课堂练习集的逐题可见性配置：qid -> {showAnswer, showAnalysis}
function classroomPushItemMap(topic) {
  const p = (db.exams || []).find((x) => x.mode === 'classroom' && x.title === topic);
  const m = {};
  if (!p) return m;
  if (Array.isArray(p.items) && p.items.length) {
    p.items.forEach((i) => { m[String(i.qid)] = { showAnswer: i.showAnswer !== false, showAnalysis: i.showAnalysis !== false }; });
  } else {
    (p.questionIds || []).forEach((qid) => { m[String(qid)] = { showAnswer: true, showAnalysis: true }; });
  }
  return m;
}
// 取消发布（删除该课堂练习集）
function removeClassroomPush(id) {
  const before = (db.exams || []).length;
  db.exams = (db.exams || []).filter((e) => !(e.id === id && e.mode === 'classroom'));
  if (db.exams.length !== before) persist();
  return getClassroomPush();
}
...
function addQuestion(input) {
  ...
  // 逐选项解析（数组，与 options 一一对应）：说明每个选项为什么对 / 错在哪
  if (Array.isArray(input.optionsAnalysis) && input.optionsAnalysis.length) {
    q.optionsAnalysis = input.optionsAnalysis.map((s) => String(s == null ? '' : s).trim());
  }
  db.questions.push(q);
  persist();
  return q;
...
function updateQuestion(id, patch) {
  ...
  // 逐选项解析：数组且与 options 等长才写入，避免错位
  if (patch.optionsAnalysis !== undefined) {
    if (Array.isArray(patch.optionsAnalysis) && patch.optionsAnalysis.length === (q.options || []).length) {
      q.optionsAnalysis = patch.optionsAnalysis.map((s) => String(s == null ? '' : s).trim());
    } else if (patch.optionsAnalysis === null) {
      delete q.optionsAnalysis;
    }
  }
...
function examCategory(e) {
  ...
  const mode = (e && e.mode) || '';
  // 冲刺强化卷（stage=强化 或标题含「冲刺」）独立成类，不与真题混排
  if ((e && e.stage === '强化') || t.includes('冲刺')) return '冲刺';
  // 专项题库:独立列表
...
// 顶层大类:冲刺强化 / 专项 / 公共基础 各自独立;常识判断/政治理论/国考/省考/选调 统一归入 国省考真题
function examTop(cat) {
  if (cat === '冲刺' || cat === '冲刺强化' || cat === '专项' || cat === '公共基础') return cat === '冲刺' ? '冲刺强化' : cat;
  return '国省考真题';
}
...
function getExamQuestions(id) {
  ...
  return { id: exam.id, title: exam.title, mode: exam.mode,
    stage: exam.stage || '基础', category: examCategory(exam),
    questions: qs.map((q) => ({
    id: q.id, type: q.type, title: q.title, options: q.options,
  })) };
}
...
function createExam(input) {
  ...
  const exam = {
    ...
    // 学习阶段：'基础'（常规题库）| '强化'（考前冲刺卷）。缺省一律归入基础阶段。
...
// 阶段归一化：只接受 基础 / 强化，其余（含 undefined）一律回落 '基础'
...
// 把试卷在「基础阶段 / 强化阶段」之间移动
function setExamStage(id, stage) {
  const e = getExam(id);
  if (!e) return null;
  e.stage = normalizeStage(stage);
  persist();
  return e;
}

// 批量设置阶段：传入 { id: stage }，返回实际更新条数
function setExamStages(map) {
  let n = 0;
  Object.keys(map || {}).forEach((id) => {
    const e = getExam(id);
    if (!e) return;
    e.stage = normalizeStage(map[id]);
    n++;
  });
  if (n) persist();
  return n;
...
// 合并两份重复试卷：把源卷的全部学生记录迁移到目标卷后，再删除源卷。
// 背景：历史上导入脚本重复建卷（如「课堂习题·课堂习题·X」与「课堂习题·X」），
// 两侧都产生了真实作答，直接 deleteExam 会让这些记录变成孤儿，故必须先迁移 refId。
function mergeExams(fromId, toId) {
  const from = getExam(fromId), to = getExam(toId);
  if (!from || !to) return { error: '考试不存在' };
  if (fromId === toId) return { error: '不能合并到自身' };
  const fromTitle = String(from.title || '');
  const toTitle = String(to.title || '');
  const stat = { answers: 0, history: 0, drafts: 0, attempts: 0 };

  // 1) 原始答题流水
  Object.keys(db.studentAnswers || {}).forEach((k) => {
    (db.studentAnswers[k] || []).forEach((a) => {
      if (!a || a.refId !== fromId) return;
      a.refId = toId;
      if (!a.refTitle || a.refTitle === fromTitle) a.refTitle = toTitle;
      stat.answers++;
    });
  });

  // 2) 学生考试记录（history）
  Object.keys(db.students || {}).forEach((k) => {
    const s = db.students[k];
    ((s && s.history) || []).forEach((h) => {
      if (!h || h.refId !== fromId) return;
      h.refId = toId;
      h.refTitle = toTitle;
      stat.history++;
    });
  ...
  // 3) 未完成草稿：迁到目标卷；若目标卷已有草稿则合并答案并保留进度更靠后的一份
  Object.keys(db.drafts || {}).forEach((k) => {
    const dd = db.drafts[k];
    if (!dd || !dd[fromId]) return;
    const src = dd[fromId];
    const dst = dd[toId];
    if (!dst) {
      dd[toId] = src;
    } else {
      dst.answers = Object.assign({}, src.answers || {}, dst.answers || {});
      if ((Number(dst.idx) || 0) < (Number(src.idx) || 0)) {
        dst.idx = src.idx;
        if (src.seq) dst.seq = src.seq;
      }
    }
    delete dd[fromId];
    stat.drafts++;
  ...
  // 4) 提交序号计数器（key 形如 `exam::e155`）
  Object.keys(db.studentAttempts || {}).forEach((k) => {
    const a = db.studentAttempts[k];
    const byRef = a && a.byRef;
    if (!byRef) return;
    Object.keys(byRef).forEach((rk) => {
      const m = /^([^:]+)::(.+)$/.exec(rk);
      if (m && m[2] === fromId) {
        const nk = m[1] + '::' + toId;
        byRef[nk] = Math.max(Number(byRef[nk]) || 0, Number(byRef[rk]) || 0);
        delete byRef[rk];
        stat.attempts++;
      }
    });
  ...
  // 5) 删除源卷
  const before = db.exams.length;
  db.exams = db.exams.filter((e) => e.id !== fromId);
  persist();
  return { ok: true, fromId, toId, toTitle, removed: db.exams.length < before, stat };
}

// 题目去重：题干+选项+答案+题型完全相同的题只保留一条。
// 保留策略：优先被考试引用最多的 → 其次被学生作答/错题引用最多的 → 最后取 id 较小的。
// 删除前先把考试 questionIds 与学生 wrong 列表重映射到保留题，避免试卷丢题。
// dryRun=true 时只返回计划，不落盘。
function dedupeQuestions(opts) {
  ...
  const examRef = new Map();
  db.exams.forEach((e) => {
    new Set(e.questionIds || []).forEach((qid) => examRef.set(qid, (examRef.get(qid) || 0) + 1));
  });
  const ansRef = new Map();
  const bump = (qid) => { if (qid) ansRef.set(qid, (ansRef.get(qid) || 0) + 1); };
  Object.keys(db.studentAnswers || {}).forEach((k) => {
    (db.studentAnswers[k] || []).forEach((a) => { if (a) bump(a.qid); });
  });
  Object.keys(db.students || {}).forEach((k) => {
    ((db.students[k] || {}).wrong || []).forEach(bump);
  });
  ...
  groups.forEach((list) => {
    ...
    const sorted = list.slice().sort((a, b) => {
      const d1 = (examRef.get(b.id) || 0) - (examRef.get(a.id) || 0);
      if (d1) return d1;
      const d2 = (ansRef.get(b.id) || 0) - (ansRef.get(a.id) || 0);
      if (d2) return d2;
      return String(a.id).localeCompare(String(b.id), 'en', { numeric: true });
    });
    const keep = sorted[0];
    sorted.slice(1).forEach((q) => remap.set(q.id, keep.id));
  });
  ...
  db.exams.forEach((e) => {
    ...
    let changed = false;
    ids.forEach((qid) => {
      const nq = remap.has(qid) ? remap.get(qid) : qid;
      if (nq !== qid) changed = true;
      if (seen.has(nq)) { changed = true; return; }
      seen.add(nq); out.push(nq);
    });
    if (changed) { e.questionIds = out; remappedExams++; }
  });

  // 学生错题本重映射
  Object.keys(db.students || {}).forEach((k) => {
    const s = db.students[k];
    if (!s || !Array.isArray(s.wrong)) return;
    const out = []; const seen = new Set();
    s.wrong.forEach((qid) => {
      const nq = remap.has(qid) ? remap.get(qid) : qid;
      if (seen.has(nq)) return;
      seen.add(nq); out.push(nq);
    });
    s.wrong = out;
  });
  ...
  if (dryRun) {
    return {
      dryRun: true, groups: groups.size, wouldDelete: remap.size, wouldRemapExams: remappedExams,
      sample: Array.from(remap.entries()).slice(0, 10).map(([f, t]) => ({ from: f, to: t })),
    };
  }

  const before = db.questions.length;
  db.questions = db.questions.filter((q) => !remap.has(q.id));
  persist();
  return { ok: true, before, after: db.questions.length, deleted: before - db.questions.length, remappedExams };
}

// 提交考试答卷：评分 + 记录 + 收集错题
// questionIds 可选：传入则只对这批题评分（用于「练几题」部分作答），否则对整套题评分
function submitExam({ examId, studentKey, studentName, answers, questionIds, durationMs }) {
  ...
  // 一次提交整批记录：区分「课堂练习集」(mode:'classroom') 与「考试 / 专题」
  const isClassroom = exam.mode === 'classroom';
  // 课堂练习支持逐题控制「答案 / 解析」是否向学生公布
  const visMap = isClassroom ? classroomPushItemMap(exam.title) : {};
  recordAnswerBatch(studentKey, {
    kind: isClassroom ? 'classroom' : 'exam',
    source: isClassroom ? '课堂练习' : '考试答题',
    refId: exam.id, refTitle: exam.title,
    studentName: studentName || (db.students[studentKey] && db.students[studentKey].name) || '',
    durationMs: (typeof durationMs === 'number' && durationMs >= 0) ? durationMs : undefined,
    total, correctCount: correct, score: total > 0 ? Math.round((correct / total) * 100) : 0,
  }, qs.map((q, i) => ({
    qid: q.id, type: q.type, topic: q.topic, questionTitle: q.title, options: q.options,
    selected: ansMap[q.id], correctAnswer: q.correctAnswer, isCorrect: details[i].isCorrect,
  })));
  ...
  persist();
  try { awardExamOnce(studentKey, exam.id); awardCorrectQuestions(studentKey, '考试', details); } catch (_) {}
  ...
  return {
    ...
    wrongAdded,
    details: qs.map((q, i) => {
      const v = isClassroom ? (visMap[String(q.id)] || { showAnswer: true, showAnalysis: true }) : { showAnswer: true, showAnalysis: true };
      return {
        id: q.id,
        type: q.type,
        title: q.title,
        ...
        correctAnswer: v.showAnswer === false ? undefined : q.correctAnswer,
        analysis: v.showAnalysis === false ? '' : q.analysis,
        optionsAnalysis: (v.showAnalysis === false || !Array.isArray(q.optionsAnalysis)) ? undefined : q.optionsAnalysis,
      };
    }),
  };
}
...
function getExamStats(examId) {
  ...
  const questions = qs.map((q) => {
    ...
    return {
      ...
      type: a.type,
      analysis: q.analysis || '',
      optionsAnalysis: q.optionsAnalysis || '',
      correct: a.correct,
      total: a.total,
...
// 批量返回所有考试（refType==='exam'，含 mode='exam' 正式考试与 mode='topic' 课堂练习题）的
...
function optLabel(i) { return typeof i === 'number' ? (OPT_LABELS[i] || String(i + 1)) : String(i); }
// 把答案（单值或数组）渲染成 "B. 选项文本" 形式，便于导出后直接阅读
function optText(opts, v) {
  const arr = Array.isArray(v) ? v : (v === undefined || v === null ? [] : [v]);
  return arr.map((x) => (typeof x === 'number' && opts[x] !== undefined ? optLabel(x) + '. ' + opts[x] : String(x))).join(' | ');
}

// 给每个学生建立"原始答题流水"库：任何一次作答都完整留底（最细粒度：一题一条）。
// 一次提交（一批题）共享 batchId 与 attemptNo（该生对同一 ref 的第几次提交）。
// meta : {kind, source, refId, refTitle, cat, section, studentName, durationMs, total, correctCount, score}
// items: [{qid, type, topic, questionTitle, options, selected, correctAnswer, isCorrect}]
function recordAnswerBatch(key, meta, items) {
  key = String(key || '').trim();
  ...
  if (!db.studentAttempts) db.studentAttempts = {};
  const att = (db.studentAttempts[key] = db.studentAttempts[key] || { seq: 0, byRef: {} });
  const m = meta || {};
  const refKey = String(m.kind || 'unknown') + '::' + String(m.refId || '');
  att.byRef[refKey] = (att.byRef[refKey] || 0) + 1;
  att.seq = (att.seq || 0) + 1;
  const now = Date.now();
  const base = {
    ts: now,
    tsISO: new Date(now).toISOString(),
    batchId: 'b' + now + '-' + att.seq,
    attemptNo: att.byRef[refKey],   // 第几次提交（针对同一考试 / 练习）
    attemptSeq: att.seq,            // 该生全局第几次提交
    studentKey: key,
    studentName: m.studentName || '',
    kind: m.kind || '',             // exam|classroom|law|wrong|memory|morningHomework|homework|room
    source: m.source || '',         // 来源模块（中文）
    refId: m.refId || '', refTitle: m.refTitle || '',
    cat: m.cat || '', section: m.section || '',
    total: m.total, correctCount: m.correctCount, score: m.score,
    durationMs: m.durationMs,
  };
  items.forEach((it) => {
    const opts = (it && it.options) || [];
    db.studentAnswers[key].push(Object.assign({}, base, {
      qid: it.qid, type: it.type, topic: it.topic || '',
      questionTitle: it.questionTitle || '', options: opts,
      selected: it.selected, selectedText: optText(opts, it.selected),
      correctAnswer: it.correctAnswer, correctText: optText(opts, it.correctAnswer),
...
// 单条记录（兼容旧调用）：内部转为批量，同样享有第几次提交等全部字段
function recordAnswerEvent(key, ev) {
  const e = ev || {};
  const q = getQuestion(e.qid);
  recordAnswerBatch(key, {
    kind: e.kind, source: e.source, refId: e.refId, refTitle: e.refTitle,
    cat: e.cat, section: e.section, studentName: e.studentName,
    durationMs: e.durationMs, total: e.total, correctCount: e.correctCount, score: e.score,
  }, [{
    qid: e.qid, type: e.type, topic: (q && q.topic) || e.topic,
    questionTitle: (q && q.title) || e.questionTitle,
    options: (q && q.options) || e.options,
    selected: e.selected, correctAnswer: e.correctAnswer, isCorrect: e.isCorrect,
  }]);
}

// 课堂房间实时答题留底（学生标识用入房时填写的 name，即 studentKey）
function recordRoomAnswer(room, roomId, studentId, answer) {
  if (!room || !room.students || !room.students[studentId]) return;
  const sKey = String(room.students[studentId].name || '').trim();
  if (!sKey) return;
  const idx = room.currentQuestion;
  const q = (room.questions || [])[idx];
  if (!q) return;
  const isCorrect = isAnswerCorrect(q, answer);
  recordAnswerEvent(sKey, {
    kind: 'room', source: '课堂房间',
    refId: roomId, refTitle: '课堂房间 ' + roomId + '（第 ' + (idx + 1) + ' 题）',
    studentName: sKey,
    qid: q.id, type: q.type, questionTitle: q.title, options: q.options,
    selected: answer, correctAnswer: q.correctAnswer, isCorrect,
    total: 1, correctCount: isCorrect ? 1 : 0, score: isCorrect ? 100 : 0,
  });
}

// 学生答题流水查询（支持按模块/练习/题型/时间/对错过滤），按时间倒序
function getStudentAnswerLog(key, opt) {
  key = String(key || '').trim();
  const o = opt || {};
  let out = ((db.studentAnswers && db.studentAnswers[key]) || []).slice();
  if (o.kind) out = out.filter((r) => r.kind === o.kind);
  if (o.refId) out = out.filter((r) => String(r.refId) === String(o.refId));
  if (o.qid) out = out.filter((r) => String(r.qid) === String(o.qid));
  if (o.topic) out = out.filter((r) => r.topic === o.topic);
  if (o.from) out = out.filter((r) => r.ts >= Number(o.from));
  if (o.to) out = out.filter((r) => r.ts <= Number(o.to));
  if (o.onlyWrong) out = out.filter((r) => r.isCorrect === false);
  out.sort((a, b) => (b.ts - a.ts) || ((b.attemptSeq || 0) - (a.attemptSeq || 0)));
  const lim = Number(o.limit);
  if (lim > 0) out = out.slice(0, lim);
  return out;
}

// 学生答题档案汇总：总提交次数、正确率、各模块/专题分布、逐次提交成绩
function getStudentAnswerSummary(key) {
  key = String(key || '').trim();
  const list = (db.studentAnswers && db.studentAnswers[key]) || [];
  const stu = db.students && db.students[key];
  const byKind = {}, byTopic = {}, byRef = {}, batches = {};
  let answered = 0, correct = 0;
  list.forEach((r) => {
    answered++;
    if (r.isCorrect) correct++;
    const k = r.kind || 'unknown';
    (byKind[k] = byKind[k] || { kind: k, source: r.source || '', answered: 0, correct: 0 }).answered++;
    if (r.isCorrect) byKind[k].correct++;
    const t = r.topic || '未分类';
    (byTopic[t] = byTopic[t] || { topic: t, answered: 0, correct: 0 }).answered++;
    if (r.isCorrect) byTopic[t].correct++;
    const rk = (r.kind || '') + '::' + (r.refId || '');
    const ref = (byRef[rk] = byRef[rk] || { kind: r.kind, refId: r.refId, refTitle: r.refTitle, batches: {}, answered: 0, correct: 0, lastTs: 0 });
    ref.batches[r.batchId] = 1;
    ref.answered++;
    if (r.isCorrect) ref.correct++;
    if (r.ts > ref.lastTs) ref.lastTs = r.ts;
    if (r.batchId && !batches[r.batchId]) {
      batches[r.batchId] = {
        batchId: r.batchId, ts: r.ts, tsISO: r.tsISO, kind: r.kind, source: r.source,
        refId: r.refId, refTitle: r.refTitle, attemptNo: r.attemptNo, attemptSeq: r.attemptSeq,
        total: r.total, correctCount: r.correctCount, score: r.score, durationMs: r.durationMs, count: 0,
      };
    }
    if (r.batchId) batches[r.batchId].count++;
  });
  const pct = (a, b) => (b > 0 ? Math.round((a / b) * 100) : 0);
  return {
    studentKey: key,
    studentName: (stu && stu.name) || (list[0] && list[0].studentName) || '',
    ...
    totalSubmissions: Object.keys(batches).length,
    byKind: Object.keys(byKind).map((k) => Object.assign(byKind[k], { accuracy: pct(byKind[k].correct, byKind[k].answered) })),
    byTopic: Object.keys(byTopic).map((t) => Object.assign(byTopic[t], { accuracy: pct(byTopic[t].correct, byTopic[t].answered) })).sort((a, b) => b.answered - a.answered),
    byRef: Object.keys(byRef).map((k) => {
      const x = byRef[k];
      return { kind: x.kind, refId: x.refId, refTitle: x.refTitle, attempts: Object.keys(x.batches).length, answered: x.answered, correct: x.correct, accuracy: pct(x.correct, x.answered), lastTs: x.lastTs, lastTsISO: x.lastTs ? new Date(x.lastTs).toISOString() : '' };
    }).sort((a, b) => b.lastTs - a.lastTs),
    submissions: Object.keys(batches).map((k) => batches[k]).sort((a, b) => b.ts - a.ts),
  };
}
...
// 姓名合法性校验（改名 / 注册统一入口）
// 背景：客户端若以非 UTF-8（如 Windows GBK 控制台）提交中文，服务端 JSON 解析会把无效字节
// 替换成 U+FFFD（replacement character）后静默落库，产生无法还原的乱码姓名，且接口仍返回成功，
// 极具迷惑性。历史上已有账号因此损坏，故在此显式拦截，让调用方立刻感知而非写入脏数据。
const NAME_MAX_LEN = 20;
const NAME_REPLACEMENT_CHAR = '\uFFFD';
function validateStudentName(nm) {
  if (!nm) return '姓名不能为空';
  if (nm.length > NAME_MAX_LEN) return `姓名过长（最多 ${NAME_MAX_LEN} 字）`;
  if (nm.indexOf(NAME_REPLACEMENT_CHAR) >= 0) return '姓名含乱码字符（编码异常），请以 UTF-8 重新提交';
  if (/[\u0000-\u001F\u007F]/.test(nm)) return '姓名含非法控制字符';
  return null;
}
...
function updateStudentName(key, name) {
  ...
  const err = validateStudentName(nm);
...
function submitWrongPractice({ studentKey, answers }) {
  ...
  persist();
  try { awardCorrectQuestions(studentKey, '错题练习', results); } catch (_) {}
  return {
    total: results.length,
    correct: results.filter((r) => r.isCorrect).length,
    removed,
...
function reviewMemory(studentKey, answers, durationMs) {
  const stu = getStudent(studentKey);
  ...
  if (results.length) {
    ...
    recordAnswerBatch(studentKey, {
      kind: 'memory', source: '记忆模式',
      refId: 'memory', refTitle: '记忆模式复习',
      studentName: (stu && stu.name) || '',
      durationMs: (typeof durationMs === 'number' && durationMs >= 0) ? durationMs : undefined,
      total: results.length, correctCount,
      score: results.length ? Math.round((correctCount / results.length) * 100) : 0,
    }, results.map((r) => {
      const q = getQuestion(r.qid) || {};
      return {
        qid: r.qid, type: r.type, topic: q.topic, questionTitle: r.title, options: r.options,
        selected: ansMap[r.qid], correctAnswer: r.correctAnswer, isCorrect: r.isCorrect,
      };
    }));
  }
  persist();
  try { awardCorrectQuestions(studentKey, '记忆复习', results); } catch (_) {}
...
function reviewSpWord(studentKey, wordId, grade) {
  ...
  card.updatedAt = now;
  persist();
  if (grade === 'remember') { try { awardCorrectQuestions(studentKey, '规范词', [{ qid: wordId, isCorrect: true }]); } catch (_) {} }
...
function reviewShenlun(studentKey, results) {
  ...
  try { awardCorrectQuestions(studentKey, '申论', out); } catch (_) {}
...
// 教师端「课堂题目」统一题库：把「课堂习题（按专题）」与「法律常识（按类别→部分）」
// 合并成同一组分组，供同一界面浏览、勾选、批量设置可见性并一次性发布。
// 每个分组带 source 标记，发布时按 source 分别投递到对应后端。
function getUnifiedClassroomBank() {
  const groups = [];
  // 1) 课堂习题：按 topic 分组
  const byTopic = {};
  (db.questions || []).forEach((q) => {
  ...
  const lawTree = {};
  (db.lawQuestions || []).forEach((q) => {
    if (!lawTree[q.cat]) lawTree[q.cat] = {};
    (lawTree[q.cat][q.section] = lawTree[q.cat][q.section] || []).push(q);
  });
  Object.keys(lawTree).sort().forEach((cat) => {
    Object.keys(lawTree[cat]).sort().forEach((section) => {
      groups.push({
        key: 'law::' + cat + '|' + section,
        source: 'law',
        sourceLabel: '法律常识',
        group: cat + ' · ' + section,
        topic: '', cat, section,
        count: lawTree[cat][section].length,
        questions: lawTree[cat][section].map((q) => ({
          id: q.id, type: q.type, title: q.title, options: q.options || [],
          correctAnswer: q.correctAnswer, analysis: q.analysis || '', topic: cat,
        })),
      });
    });
  });
...
function submitLaw(studentKey, cat, section, answers) {
  ...
  persist();
  try { awardCorrectQuestions(studentKey, '法律练习', details); } catch (_) {}
...
function registerStudent({ email, name, password }) {
  ...
  const nm = String(name || '').trim();
  const nameErr = validateStudentName(nm);
...
function submitMorningHomework({ homeworkId, studentKey, studentName, answers }) {
  ...
  persist();
  try { awardCorrectQuestions(studentKey, '早读作业', details); } catch (_) {}
  return { total: qs.length, correct, score: qs.length ? Math.round((correct / qs.length) * 100) : 0, details: qs.map((q, i) => ({ id: q.id, title: q.title, options: q.options, selected: details[i].selected, correctAnswer: q.correctAnswer, isCorrect: details[i].isCorrect, analysis: q.analysis, optionsAnalysis: q.optionsAnalysis })) };
}
...
function getAIConfig() {
  db.meta = db.meta || {};
  // 默认使用「硅基流动（SiliconFlow）」云端大模型：OpenAI 兼容接口
  // （base_url=https://api.siliconflow.cn/v1），国内稳定、价格低、无需本机显卡；
  // 同时在教师端「AI 分析配置」中也可切回火山方舟（Ark）或本地 Ollama。
  // 若改为本地 Ollama（http://127.0.0.1:11434/v1），则可免 key 开箱即用。
  const def = db.meta.aiConfig || {};
  const endpoint = def.endpoint || 'https://api.siliconflow.cn/v1';
  // 关键：当用户尚未显式配置过时，若默认/当前 endpoint 为本地自托管服务，
  // 直接默认 enabled=true，让本地 AI 开箱即用（无需先点保存配置）。
  // 云端服务（硅基流动/火山方舟等）仍要求用户明确启用并填 key。
  ...
  return {
    endpoint,
    key: def.key || '',
    model: def.model || 'deepseek-ai/DeepSeek-V4-Flash',
    enabled,
  };
}
...
// ============ AI 对话历史（按学生/访客持久保存） ============
function getAIChat(key) {
  db.aiChats = db.aiChats || {};
  const k = String(key || 'guest').trim() || 'guest';
  return db.aiChats[k] || [];
}
function appendAIChat(key, entries) {
  db.aiChats = db.aiChats || {};
  const k = String(key || 'guest').trim() || 'guest';
  if (!Array.isArray(db.aiChats[k])) db.aiChats[k] = [];
  for (const e of (entries || [])) {
    if (e && e.role && typeof e.content === 'string') db.aiChats[k].push({ role: e.role, content: e.content, ts: e.ts || Date.now() });
  }
  const MAX = 40; // 单用户最多保留最近 40 条，避免无限增长
  if (db.aiChats[k].length > MAX) db.aiChats[k] = db.aiChats[k].slice(db.aiChats[k].length - MAX);
  persist();
  return db.aiChats[k];
}
function clearAIChat(key) {
  db.aiChats = db.aiChats || {};
  const k = String(key || 'guest').trim() || 'guest';
  delete db.aiChats[k];
  persist();
...
// ========== AI 模型注册表 + 学生积分 ==========
// 学生端可在「DeepSeek 极速（免费）」与「DeepSeek 旗舰（消耗积分）」之间自选；
// 旗舰档为更强模型，积分通过「完成考试」与「首次答对题目」赚取，按题去重防止刷分。
const AI_MODEL_REGISTRY = {
  'deepseek-ai/DeepSeek-V4-Flash': { label: 'DeepSeek 极速', cost: 0, premium: false },
  'deepseek-ai/DeepSeek-V4-Pro': { label: 'DeepSeek 旗舰', cost: 10, premium: true },
};
const STUDENT_FREE_MODEL = 'deepseek-ai/DeepSeek-V4-Flash';
const POINT_RULES = { EXAM_REWARD: 10, PRACTICE_PER_CORRECT: 2, DEEPSEEK_COST: 10 };

function normPointKey(key) {
  const k = String(key || 'guest').trim().toLowerCase();
  return k || 'guest';
}
function getPoints(key) {
  db.points = db.points || {};
  const k = normPointKey(key);
  if (!db.points[k]) db.points[k] = { balance: 0, examDone: {}, correctQ: {}, log: [] };
  return db.points[k];
}
function _addPoints(key, delta, reason, ref) {
  const p = getPoints(key);
  p.balance = Math.max(0, (p.balance || 0) + delta);
  p.log = p.log || [];
  p.log.push({ ts: Date.now(), delta, reason: reason || '', ref: ref || '', balance: p.balance });
  if (p.log.length > 300) p.log = p.log.slice(-300);
  persist();
  return p.balance;
}
function awardExamOnce(key, examId) {
  const p = getPoints(key);
  if (p.examDone[String(examId)]) return p.balance;
  p.examDone[String(examId)] = true;
  return _addPoints(key, POINT_RULES.EXAM_REWARD, '完成考试', String(examId));
}
function awardCorrectQuestions(key, kind, items) {
  const p = getPoints(key);
  let gained = 0;
  (items || []).forEach((it) => {
    if (!it || !it.isCorrect) return;
    const qid = it.qid || it.id;
    if (!qid || p.correctQ[String(qid)]) return;
    p.correctQ[String(qid)] = true;
    gained += POINT_RULES.PRACTICE_PER_CORRECT;
  });
  if (gained > 0) return _addPoints(key, gained, '答对题目(' + (kind || '练习') + ')', kind || '');
  return p.balance;
}
function spendPoints(key, amount, reason) {
  const p = getPoints(key);
  if ((p.balance || 0) < amount) return { ok: false, balance: p.balance || 0 };
  _addPoints(key, -amount, reason || '消费', null);
  return { ok: true, balance: p.balance };
}
function refundPoints(key, amount, reason) {
  return _addPoints(key, amount, reason || '退款', null);
}
function getAIModels(key) {
  const p = getPoints(key);
  const isGuest = !key || key === 'guest' || !getStudent(key);
  return {
    default: STUDENT_FREE_MODEL,
    models: Object.keys(AI_MODEL_REGISTRY).map((id) => ({
      id,
      label: AI_MODEL_REGISTRY[id].label,
      cost: AI_MODEL_REGISTRY[id].cost,
      premium: !!AI_MODEL_REGISTRY[id].premium,
    })),
    points: p.balance || 0,
    isGuest,
  };
...
module.exports = {
  ...
  // 考试
  listExams, getExam, getExamQuestions, createExam, deleteExam, mergeExams, dedupeQuestions, submitExam,
  setExamStage, setExamStages, normalizeStage,
  ...
  // 课堂练习（课堂模块：按专题浏览题库 → 教师勾选发布练习集）
  getClassroomBank, getClassroomPush, addClassroomPush, removeClassroomPush, getUnifiedClassroomBank,
  // 学生答题流水（每人一库，含第几次提交）
  recordAnswerEvent, recordAnswerBatch, recordRoomAnswer, getStudentAnswerLog, getStudentAnswerSummary,
  // 学生/错题
  ...
  // AI 对话助手
  getAIChat, appendAIChat, clearAIChat,
  getPoints, getAIModels, spendPoints, refundPoints, awardExamOnce, awardCorrectQuestions,
  AI_MODEL_REGISTRY, STUDENT_FREE_MODEL,
};
