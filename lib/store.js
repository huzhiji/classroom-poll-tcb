...
let db = {
  ...
  lawPushs: [],     // 法律课堂推送列表 [{id,cat,section,showAnswer,showAnalysis,pushedAt}]
  lawQStats: {},    // 每题作答统计 {qid:{cat,section,correct,total}}
  meta: { qSeq: 0, eSeq: 0 },
};
...
function getSpDaily(studentKey) {
  ...
  if (newLimit > 0) {
    for (const w of db.spWords) {
      ...
      if (!prog[w.id]) newCards.push({ id: w.id, cat: w.cat, scene: w.scene });
    }
  }
...
function seedLawQuestions() {
  ...
  try {
    LAW_SEED_FILES.forEach((f) => {
      ...
      arr.forEach((q) => {
        if (!q || !q.id) return;
        if (!(db.lawQuestions || []).some((x) => x.id === q.id)) {
    ...
    console.log(`[store] 法律题库合并完成：共 ${db.lawQuestions.length} 题（本次新增 ${added.length} 题）`);
...
function getLawPush() {
  // 附带 count：学生端实际可见题数（qids 子集或全部分）
  return (db.lawPushs || []).map((p) => {
    const qids = Array.isArray(p.qids) && p.qids.length ? new Set(p.qids) : null;
    const count = (db.lawQuestions || []).filter((q) => q.cat === p.cat && q.section === p.section && (!qids || qids.has(q.id))).length;
    return Object.assign({}, p, { count });
  });
}
// 按部分筛选已发布题目 id 集（null = 该部分全部题目）
function lawPushQids(cat, section) {
  const p = (db.lawPushs || []).find((x) => x.cat === cat && x.section === section);
  return p && Array.isArray(p.qids) && p.qids.length ? new Set(p.qids) : null;
}
// 追加/更新推送（同一部分重复推送则更新可见性配置；qids 为空数组表示清空为该部分无题）
function addLawPush(items) {
  if (!db.lawPushs) db.lawPushs = [];
  (items || []).forEach((it) => {
    const cfg = {
      cat: String(it.cat || ''), section: String(it.section || ''),
      showAnswer: it.showAnswer !== false, showAnalysis: it.showAnalysis !== false,
      pushedAt: Date.now(),
    };
    // 只保留该部分真实存在的题目 id
    if (Array.isArray(it.qids)) {
      const valid = new Set((db.lawQuestions || []).filter((q) => q.cat === cfg.cat && q.section === cfg.section).map((q) => q.id));
      cfg.qids = it.qids.filter((id) => valid.has(String(id)));
    }
    const idx = db.lawPushs.findIndex((p) => p.cat === cfg.cat && p.section === cfg.section);
    if (idx >= 0) {
      const merged = Object.assign({}, db.lawPushs[idx], cfg);
      db.lawPushs[idx] = merged;
...
// 学生取题：按该部分推送配置（qids + 可见性）决定返回哪些题、是否带正确答案/解析
// full=true 时无条件返回完整数据（教师端使用）
function getLawQuestions(cat, section, full) {
  const qids = lawPushQids(cat, section);
  const p = (db.lawPushs || []).find((x) => x.cat === cat && x.section === section);
  const showAnswer = full ? true : (p ? p.showAnswer : false);
  const showAnalysis = full ? true : (p ? p.showAnalysis : false);
  return db.lawQuestions.filter((q) => (!cat || q.cat === cat) && q.section === section && (!qids || qids.has(q.id))).map((q) => {
...
// 判分 + 每题统计 + 答错自动进错题本：只判已推送题目，按推送配置决定结果里是否带答案/解析
function submitLaw(studentKey, cat, section, answers) {
  const qids = lawPushQids(cat, section);
  const qs = db.lawQuestions.filter((q) => (!cat || q.cat === cat) && q.section === section && (!qids || qids.has(q.id)));
  const p = (db.lawPushs || []).find((x) => x.cat === cat && x.section === section);
  const showAnswer = p ? p.showAnswer : false;
  const showAnalysis = p ? p.showAnalysis : false;
  let correct = 0;
  const wrongQids = [];
  const details = [];
  if (!db.lawQStats) db.lawQStats = {};
  qs.forEach((q) => {
    const sel = answers && answers[q.id];
    const ok = isAnswerCorrect(q, sel);
    if (ok) correct++; else wrongQids.push(q.id);
    const st = db.lawQStats[q.id] || { cat: q.cat, section: q.section, correct: 0, total: 0 };
    st.total++;
    if (ok) st.correct++;
    db.lawQStats[q.id] = st;
    const d = { id: q.id, title: q.title, options: q.options, type: q.type, isCorrect: ok, selected: sel };
    if (showAnswer) d.correctAnswer = q.correctAnswer;
    if (showAnalysis) d.analysis = q.analysis;
    details.push(d);
  });
  const stu = ensureStudent(studentKey, studentKey);
...
function getLawStats() {
  const qStats = db.lawQStats || {};
  const secMap = {};
  const questions = [];
  Object.keys(qStats).forEach((qid) => {
    const st = qStats[qid];
    const q = db.lawQuestions.find((x) => x.id === qid);
    const key = st.cat + '|' + st.section;
    if (!secMap[key]) secMap[key] = { cat: st.cat, section: st.section, correct: 0, total: 0 };
    secMap[key].correct += st.correct;
    secMap[key].total += st.total;
    questions.push({
      id: qid, cat: st.cat, section: st.section, title: q ? q.title : '',
      correct: st.correct, total: st.total,
      percent: st.total ? Math.round((st.correct / st.total) * 100) : 0,
    });
  });
  const sections = Object.keys(secMap).map((k) => {
    const s = secMap[k];
  ...
  questions.sort((a, b) => a.cat.localeCompare(b.cat) || a.section.localeCompare(b.section) || (a.percent - b.percent));
