...
function getClassroomPush() {
  return (db.exams || []).filter((e) => e.mode === 'classroom').map((e) => ({
    id: e.id, title: e.title, count: (e.questionIds || []).length, pushedAt: e.createdAt || 0,
  }));
}
...
function addClassroomPush(list) {
  (list || []).forEach((g) => {
    ...
    const qids = (g.qids || []).map(String).filter((id) => valid.has(id));
    if (!qids.length) return;
    const exam = db.exams.find((e) => e.mode === 'classroom' && e.title === topic);
    if (exam) { exam.questionIds = qids; }
    else db.exams.push({ id: nowId('e', 'eSeq'), title: topic, mode: 'classroom', questionIds: qids, createdAt: Date.now() });
  });
  persist();
...
module.exports = {
  ...
  getClassroomBank, getClassroomPush, addClassroomPush, removeClassroomPush,
  // 学生答题流水（每人一库，含第几次提交）
