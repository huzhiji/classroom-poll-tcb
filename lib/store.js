...
function getAIConfig() {
  ...
  return {
    endpoint,
    key: def.key || '',
    model: def.model || 'deepseek-ai/DeepSeek-V3.2',
    enabled,
  };
}
...
// ========== AI 模型注册表 + 学生积分 ==========
// 学生端可在「Qwen 通识（免费）」与「DeepSeek 深度（消耗积分）」之间自选；
// 积分通过「完成考试」与「首次答对题目」赚取，按题去重防止刷分。
const AI_MODEL_REGISTRY = {
  'Qwen/Qwen3.5-122B-A10B': { label: 'Qwen 通识', cost: 0, premium: false },
  'deepseek-ai/DeepSeek-V3.2': { label: 'DeepSeek 深度', cost: 10, premium: true },
};
const STUDENT_FREE_MODEL = 'Qwen/Qwen3.5-122B-A10B';
