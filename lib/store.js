...
// 顶层大类：常识判断 / 公共基础 / 专题 各自独立；其余真题统一归到 国省考真题
...
function getExamProgress(studentKey) {
  ...
  (stu ? stu.history || [] : []).forEach((h) => {
    if (h.refType === 'exam' && h.refId) doneSet.add(String(h.refId));
  });
  const byTop = {};
  db.exams.forEach((e) => {
...
module.exports = {
  ...
  saveDraft, getDraft, clearDraft, listDrafts, getExamProgress, examCategory, examTop,
  // 学生/错题
  ...
  // 注册 / 登录 / 提醒
  registerStudent, loginStudent, getReminderRecipients,
  // 课程
